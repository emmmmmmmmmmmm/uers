module.exports = {
	'zh_CN': {
		'uers_process': '任务列表',
		'optionItems': '流转意见',
		'processInformation': '申请信息',
		'basicInformation': '基本信息',
		'approvalDetail': '流程详情',
		'toDealTask': '待处理的',
		'mySubmission': '我提交的',
		'flowToMe': '流经我的',
		'taskId': '任务编号',
		'applyUser': '申请人',
		'tableName': '补录表名',
		'belongLine': '归属条线',
		'taskUser': '提交人',
		'startTime': '提交日期',
		'taskStatus': '任务状态',
		'currentUsers': '当前审批人',
		'operation': '操作',
		'approval': '审批',
		'comeback': '退回',
		'back': '返回',
		'agree': '同意',
		'submit': '提交',
		'Please': {
			'input': '请输入',
			'select': '请选择'
		},
		'taskProcessed': '全部',
		'keyword': '关键字',
		'project/name': '项目编号/名称',
		'date': '日期',
		'startDate': '开始日期',
		'endDate': '结束日期',
		'query': '查询',
		'reset': '重置',
		'processedTime': '处理完成时间',
		'view': '查看',
		'Waiting for loading': '正在加载中...',
		'inAll': '总共',
		'projects': '个项目',
		'have chosen': '已选择',
		'tableName':'补录表名',
		'belongLine':'归属条线',
		'inProcess':'流程中',
		'completed':'已完成',
		'unknow':'未知',
		"delete": "删除",
		"confirmDelete": "确认是否删除",
		"warning":"提醒",
		"confirm":"确认",
		"deleteSuccess":"删除成功",
		"deleteFailed":"删除失败",
		"confirmApprove":"确认审批通过",
		"approveSuccess":"审批成功",
		"approveCommentPrompt":"请输入审批意见",
		"approveCommentPlaceholder":"请输入审批意见（必填，最多1000字）",
		"approveCommentRequired":"审批意见不能为空",
		"approveCommentTooLong":"审批意见不能超过1000字",
		"approveFailed":"审批失败",
		"reject":"驳回",
		"confirmReject":"确认驳回",
		"rejectCommentPrompt":"请输入驳回意见",
		"rejectCommentPromptPlaceHolder":"请输入驳回意见(必填，最多1000字)",
		"rejectCommentRequired":"驳回意见不能为空",
		"rejectCommentTooLong":"驳回意见不能超过1000字",
		"rejectSuccess":"驳回成功",
		"rejectFailed":"驳回失败",
		rejectDisabled:{
			"handling":"经办状态不可驳回"
		},
		"prompt":"提示",
		"NO.":"序号",
		"exportExcel":"导出excel",
		"exporting":"正在导出...",
		"exportSuccess":"导出成功",
		"exportFailed":"导出失败",
		"dataStatus":"数据状态",
		"valid":"有效",
		"invalid":"无效",
		deleteDisabled:{
			"review":"当前任务处于复核节点，不可删除",
			"initialApproval":"当前任务处于初始审核节点，不可删除",
			"finalApproval":"当前任务处于最终审核节点，不可删除",
			"completed":"当前任务已完成，不可删除",
			"notAllowed":"当前任务状态不允许删除"
		},
		"noAttachment":"没有可下载的附件",
		"viewTaskFailed":"查看任务失败"
	},	
	'en-US': {
		'uers_process': 'taskList',
		'optionItems': 'optionItems',
		'processInformation': 'processInformation',
		'basicInformation': 'basicInformation',
		'approvalDetail': 'approvalDetail',
		'toDealTask': 'toDealTask',
		'mySubmission': 'mySubmission',
		'flowToMe': 'flowToMe',
		'taskId': 'taskId',
		'applyUser': 'applyUser',
		'tableName': 'tableName',
		'belongLine': 'belongLine',
		'taskUser': 'taskUser',
		'startTime': 'startTime',
		'taskStatus': 'taskStatus',
		'currentUsers': 'currentUsers',
		'operation': 'operation',
		'approval': 'approval',
		'comeback': 'comeback',
		'back': 'back',
		'agree': 'agree',
		'submit': 'submit',
		'Please': {
			'input': 'input',
			'select': 'select'
		},
		'taskProcessed': 'all',
		'keyword': 'keyword',
		'project/name': 'project/name',
		'date': 'date',
		'startDate': 'startDate',
		'endDate': 'endDate',
		'query': 'query',
		'reset': 'reset',
		'processedTime': 'processedTime',
		'view': 'view',
		'Waiting for loading': 'Waiting for loading...',
		'inAll': 'inAll',
		'projects': 'projects',
		'have chosen': 'have chosen',
		'tableName':'tableName',
		'belongLine':'belongLine',
		'inProcess':'inProcess',
		'completed':'completed',
		'unknow':'unknow',
		"delete":"delete",
		"confirmDelete": "确认删除",
		"warning":"提醒",
		"confirm":"确认",
		"deleteSuccess":"deleteSuccess",
		"deleteFailed":"deleteFailed",
		"confirmApprove":"confirmApprove",
		"approveCommentPrompt":"Please enter approve comment",
		"approveCommentPlaceholder":"Please enter approve comment(required,max 1000 characters)",
		"approveCommentRequired":"Approve comment is required",
		"approveCommentTooLong":"Approve comment context exceed 1000 characters",
		"approveSuccess":"approveSuccess",
		"approveFailed":"approveFailed",
		"reject":"reject",
		"confirmReject":"Confirm Reject",
		"rejectCommentPrompt":"Please enter reject comment",
		"rejectCommentPromptPlaceHolder":"Please enter reject comment(required, max 1000 characters)",
		"rejectCommentRequired":"Reject comment is required",
		"rejectCommentTooLong":"Reject comment context exceed 1000 characters",
		"rejectSuccess":"Reject Success",
		"rejectFailed":"Reject Failed",
		rejectDisabled:{
			"handling":"Handling status cannot be rejected"
		},
		"prompt":"Prompt",
		"NO.":"NO.",
		"exportExcel":"exportExcel",
		"exporting":"exporting...",
		"exportSuccess":"Export Success",
		"exportFailed":"Export Failed",
		"dataStatus":"Data Status",
		"valid":"Valid",
		"invalid":"Invalid",
		deleteDisabled:{
			"review":"Current task is in Review stage,cannot be deleted",
			"initialApproval":"Current task is in Initial stage,cannot be deleted",
			"finalApproval":"Current task is in Final Approval stage,cannot be deleted",
			"completed":"Current task is in Final Approval stage,cannot be deleted",
			"notAllowed":"Current task status does not allow deletion"
		},
		"noAttachment":"No attachment available for download",
		"viewTaskFailed":"Failed to view task"
	}
}